...
drop function if exists public.add_home_post_comment(uuid, text);
drop function if exists public.list_home_post_comments(uuid);
drop function if exists public.list_home_posts(integer);

create function public.toggle_home_post_like(p_post_id uuid)
returns void
language plpgsql
security definer
...
create function public.add_home_post_comment(
  p_post_id uuid,
  p_body text
)
returns void
...
create function public.list_home_post_comments(p_post_id uuid)
returns table (
  id uuid,
  author_id uuid,
  author_name text,
...
create function public.list_home_posts(p_limit integer default 50)
returns table (
  id uuid,
  author_id uuid,
  author_name text,
